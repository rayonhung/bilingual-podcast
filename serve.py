...
def mp3_chunks(data, max_bytes, max_seconds=None, first_max_seconds=None):
    """
...
def split_audio(audio_bytes):
    ...
    """
    chunks = mp3_chunks(audio_bytes, CHUNK_BYTES, CHUNK_SECONDS, FIRST_CHUNK_SECONDS)
    if chunks:
        return chunks
...
def _transcribe_one_with_split_fallback(audio_bytes, filename, model, key, attempts=1):
    try:
        ...
        parts = mp3_chunks(audio_bytes, CHUNK_BYTES, FALLBACK_CHUNK_SECONDS)
...
def job_start(data):
    ...
    with JOBS_LOCK:
        ...
        for jid in [k for k, v in JOBS.items() if now - v["ts"] > 1800]:
            del JOBS[jid]
        while len(JOBS) >= 4:
            del JOBS[min(JOBS, key=lambda k: JOBS[k]["ts"])]
        JOBS[job_id] = {"chunks": chunks, "name": name, "ts": now}
